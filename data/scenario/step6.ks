*a
[eval exp="f.step=6" ][eval exp="f.feeling=0" ][eval exp="f.act='non'" ]
...
[eval exp="f.d_save1_n='搭配1'" ][eval exp="f.d_save2_n='搭配2'" ][eval exp="f.d_save3_n='搭配3'" ][eval exp="f.d_save4_n='搭配4'" ]
[eval exp="f.d_save5_n='搭配5'" ][eval exp="f.d_save6_n='搭配6'" ][eval exp="f.d_save7_n='搭配7'" ][eval exp="f.d_save8_n='搭配8'" ]
[eval exp="f.d_save9_n='搭配9'" ][eval exp="f.d_save10_n='搭配10'" ][eval exp="f.d_save11_n='搭配11'" ][eval exp="f.d_save12_n='搭配12'" ]
[eval exp="f.d_save13_n='搭配13'" ][eval exp="f.d_save14_n='搭配14'" ][eval exp="f.d_save15_n='搭配15'" ][eval exp="f.d_save16_n='搭配16'" ]
[eval exp="f.d_save17_n='搭配17'" ][eval exp="f.d_save18_n='搭配18'" ][eval exp="f.d_save19_n='搭配19'" ][eval exp="f.d_save20_n='搭配20'" ]
...
[if exp="f.talk=='dress'" ][jump storage="talk/dress_talk.ks" target="*check" ][endif]
...
*before_menu
#
[hide_skip][if exp="f.act<=6" ][else][jump target="*night" ][endif]
[if exp="f.lust_touch<=0 || f.last_act=='dinner' || f.last_act=='cafe' || f.last_act=='hiroba' || f.last_act=='wood' || f.last_act=='market' || f.last_act=='work' || f.last_act=='shop' || f.last_act=='tea'" ]
[eval exp="f.lust_touch=0" ][else][eval exp="f.lust_touch=f.lust_touch-1" ][endif]
[if exp="f.lust_touch>=4 && f.lust>=200" ][eval exp="f.mood='lust'" ][else][eval exp="f.mood='calm'" ][endif]
...
*night
#
[show_skip][eval exp="f.act='non'" ][eval exp="f.sexless=f.sexless+1" ]
...
[elsif exp="f.lust<=300" ][jump target="*x" ][elsif exp="f.lust<=800" ][jump target="*xx" ][else][jump target="*xxx" ][endif]

*x
[if exp="f.sexless>=4 && f.sexless_c==0" ][eval exp="f.sexless=0" ][eval exp="f.sexless_c=1" ][jump target="*day_end_h" ]
[elsif exp="f.sexless>=4 && f.sexless_c==1" ][eval exp="f.sexless=0" ][eval exp="f.sexless_c=2" ][day_reset]
…[p]（已經到了晚上了…準備睡覺吧）[p][jump storage="H/sexless1.ks" target="*sexless1" ]
[elsif exp="f.sexless>=4 && f.sexless_c==2" ][eval exp="f.sexless=0" ][eval exp="f.sexless_c=3" ][day_reset]
…[p]（已經到了晚上了…準備睡覺吧）[p][jump storage="H/sexless2.ks" target="*sexless2" ]
...
*xx
[if exp="f.sexless>=3 && f.sexless_c==0" ][eval exp="f.sexless=0" ][eval exp="f.sexless_c=1" ][jump target="*day_end_h" ]
[elsif exp="f.sexless>=3 && f.sexless_c==1" ][eval exp="f.sexless=0" ][eval exp="f.sexless_c=2" ][day_reset]
…[p]（已經到了晚上了…準備睡覺吧）[p][jump storage="H/sexless1.ks" target="*sexless1" ]
[elsif exp="f.sexless>=3 && f.sexless_c==2" ][eval exp="f.sexless=0" ][eval exp="f.sexless_c=3" ][day_reset]
…[p]（已經到了晚上了…準備睡覺吧）[p][jump storage="H/sexless2.ks" target="*sexless2" ]
...
*xxx
[if exp="f.sexless>=2 && f.sexless_c==0" ][eval exp="f.sexless=0" ][eval exp="f.sexless_c=1" ]
[jump target="*day_end_h" ]
[elsif exp="f.sexless>=2 && f.sexless_c==1" ][eval exp="f.sexless=0" ][eval exp="f.sexless_c=2" ][day_reset]
…[p]（已經到了晚上了…準備睡覺吧）[p][jump storage="H/sexless1.ks" target="*sexless1" ]
[elsif exp="f.sexless>=2 && f.sexless_c==2" ][eval exp="f.sexless=0" ][eval exp="f.sexless_c=3" ][day_reset]
…[p]（已經到了晚上了…準備睡覺吧）[p][jump storage="H/sexless2.ks" target="*sexless2" ]
...
[s_clt]哇…[lr]
[s_tp]嗯、因為喝了酒、覺得有些困[p]
[elsif exp="f.daily_out=='cafe'" ]
[s_st]今天咖啡店的甜品、非常好吃[lr]
[s_ss]如果有機會、再就和您一起去[p]
[elsif exp="f.daily_out=='dinner'" ]
[s_st]剛才那家店的飯真好吃[lr]
[s_ss]吃飽了就想睡覺[p]
[elsif exp="f.daily_out=='shop_d'" ]
[s_st]今天又給我買新衣服了了、謝謝您[lr]
[s_ss]明天就穿著它和[name]出門吧…如果可以的話[p]
[elsif exp="f.daily_out=='shop_a'" ]
[s_st]今天又給我買新首飾了、謝謝您[lr]
[s_ss]明天就帶上它和[name]出門吧…如果可以的話[p]
...
[s_st]泡了下午茶…來悠閒一天吧[lr]
[s_ss][name]買來的[miyage]真好吃[p]
[elsif exp="f.tea_time==1" ]
[s_st]泡了下午茶…來悠閒一天吧[p]
[elsif exp="f.daily_nade>=4" ]
[s_st]感覺今天[name]一直在摸我[lr]
[s_tp]我覺得好幸福啊…[p]
[elsif exp="f.daily_wood>=2" ]
[s_sst]今日和[name]出去真開心[lr]
[s_st]總覺得在綠茵中散步心情也會好起來[p]
[elsif exp="f.daily_market>=4" ]
[s_sst]今日和[name]出去真開心[lr]
[s_st]每次去市場那些店都有一些不一樣不管去幾次都有新鮮感[p]
[elsif exp="f.daily_hiroba>=4" ]
[s_sst]今日和[name]出去真開心[lr]
[s_st]那個廣場真是寬敞舒適啊[p]
[elsif exp="f.daily_act=='apron_sex'" ]
[s_cltp]今日的那個…[lr]
[s_tp]因為是午後去的覺得時間有一點短[p]
[elsif exp="f.daily_talk>=4" ]
[s_st]雖然今天沒做什麽、[r]
但是能和[name]在一起的話就覺得很快樂了[p]
[else][endif]
...
[s_st]差不多該睡覺了呢[p]
#
[day_reset][black]…[p][jump target="*show_bace" ]
...
[s_cclp]…[p]
#
[day_reset][jump storage="H/sexless3.ks" target="*sexless3" ]
...
*talk3
[s_sclt]雖然出門很開心、但是這樣兩人在一起也很能讓人安心呢[p]
[s_s][eval exp="f.lust=f.lust-1" ]
[jump storage="after_action.ks" target="*after_talk" ]
*talk4
[s_st]我、特別地幸福呢[lr]
[name]只要能和[name]在一起、就算想起以前的事也不會痛苦[p]
...
[jump storage="after_action.ks" target="*after_talk" ]
*talk8
[s_st]只要能和[name]在一起我就能忘掉以前的事情[p]
[s_s][jump storage="after_action.ks" target="*after_talk" ]
*talk9
...
[s_ccl][jump storage="after_action.ks" target="*after_talk" ]
*talk23
[s_t]如果再次與那個商人見面的話、一定要好好感謝他呢[lr]
...
[s_cclt]如果事件發生地再晚一些的話[r]
我和那些人恐怕下場就一樣了吧[p]
[s_ccl][jump storage="after_action.ks" target="*after_talk" ]
*talk33
[s_clt]自從我懂事的時候我就是一個人、完全不知道家人的存在[lr]
...
[s_stp]如果[name]有什麽想讓我穿的話、我一定會穿的[p]
[s_sp][jump storage="after_action.ks" target="*after_talk" ]
*talk46
...
[s_p][jump storage="after_action.ks" target="*after_talk" ]
*talk63
[s_ctp]現在…只要我和[name]在一起就只在考慮H的事情了[p]
